...
async function handleDepositedEvent(event: any, web3: Web3): Promise<void> {
    const { orderId, buyer, seller, amount, departure, destination } = event.returnValues;
    const txHash = event.transactionHash;

    console.log(`[EscrowListener] ───────────────────────────────────────`);
    console.log(`[EscrowListener] Deposited event detected!`);
    console.log(`[EscrowListener]   orderId:     ${orderId}`);
    console.log(`[EscrowListener]   buyer:       ${buyer}`);
    console.log(`[EscrowListener]   seller:      ${seller}`);
    console.log(`[EscrowListener]   amount:      ${web3.utils.fromWei(amount, 'mwei')} USDC`); // USDC has 6 decimals
    ...
    // Verify: check if this orderId matches a LISTED order in our DB
    const listings = getListings();
    const matchingOrder = listings.find(o => o.id === orderId);

    if (!matchingOrder) {
        console.log(`[EscrowListener] ⚠ No matching LISTED order for orderId: ${orderId}`);
        return;
    }
    ...
    console.log(`[EscrowListener]   Expected:    $${expectedCostUSDC.toFixed(2)} USDC`);
    console.log(`[EscrowListener]   Deposited:   $${depositedUSDC.toFixed(2)} USDC`);

    // Update order to ESCROWED
    escrowOrder(orderId, buyer, departure, destination, txHash);

    console.log(`[EscrowListener] ✅ Order ${orderId} marked ESCROWED`);
